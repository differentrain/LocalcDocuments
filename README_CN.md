...
如图所示，一元运算的按钮 NEG(求反) 和 NOT(位运算 NOT) 皆位于数字框中。MAX(当前整数类型的最大值) 、MAX(当前整数类型的最小值)、CLR(数字置0)、BKSP(退格)等按钮也在其中。 当你点击这些按钮，其所属的数字框就会执行相应的操作。而如果你按下了相关[快捷键](快捷键)，操作将会执行于当前被选中的数字框。
...
|$*\<varName\>*|引用定义于[变量列表](变量)的变量的值|如果表达式的整数类型小于64位，则此值将会被截断|
...
用来输入变量值的文本框提供了对 [表达式](表达式) 的计算功能, **不过其用于计算的整数类型是 `Int64` **. 
